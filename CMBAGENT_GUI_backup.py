...
def _history_filename(username: str | None) -> str:
    ...
    name = f"{username}_chat_history.json"
...
if st.session_state.page == "mode_select":
    ...
    if user_input:
        ...
        with st.chat_message("assistant") as assistant_message:
            ...
            exp = st.expander("🧠 Internal reasoning (optional)", expanded=False)
            handler = StreamHandler(exp)
            answer_container = st.container()          # <-- NEW: placeholder that
                                                    #     we’ll fill after thinking
  ...
  # … right after your redirect_stdout(...) block …
        ...
        # … after you’ve swept ./output/data for images …

        # now also pick up any Markdown files in ../output (one level up):
        # md_dir = os.path.join(os.path.dirname(__file__), "output")
        # md_paths = sorted(glob.glob(os.path.join(md_dir, "*.md")))
        ...
        #     # append it to to_display so it ends up in your assistant bubble
        #     filename = os.path.basename(md_path)
        #     to_display.append(("file", f"### Contents of `{filename}`\n\n" + md_text))


        # keep this assistant reply so it survives the next rerun
        ...
        if to_display:
            with answer_container:                     # <-- note the placeholder
