...
def recursive_update(
    original: dict[str, Any], update: Mapping[str, Any]
) -> dict[str, Any]:
    """
    Recursively update mapping *original* with values from *update*.
    ...
    for key, value in update.items():
        if (
            key in original
            and isinstance(original[key], Mapping)
            and isinstance(value, Mapping)
        ):
            original[key] = recursive_update(dict(original[key]), value)
        else:
            original[key] = value
    return original


def prepare_comment(comment: str) -> str:
    """
    Turn an arbitrary string into a YAML comment block.
    """
...
def _construct_defaults(loader, node):
    ...
    loaded_defaults: dict[str, Any] = {}
    for dfile in defaults_files:
        dfilename = os.path.abspath(os.path.join(folder, dfile))
        for ext in YAML_EXTENSIONS:
            candidate = dfilename + ext
            if os.path.basename(candidate) in os.listdir(os.path.dirname(candidate)):
                dfilename = candidate
                break
        else:
            raise InputSyntaxError(
...
def yaml_load(text_stream, file_name=None) -> dict[str, Any]:
...
def yaml_load_file(file_name: str | None, yaml_text: str | None = None) -> dict[str, Any]:
